...
A newborn fails metabolic screening, and whole-genome sequencing reveals a novel variant deep within an intron of an enzyme gene. The variant overlaps a DNase hypersensitive site cataloged in ENCODE, suggesting regulatory potential. But overlap tells the clinician nothing about whether this variant strengthens or weakens regulatory activity, nothing about effect magnitude, nothing about which gene expression program is affected. Genome-wide association studies have mapped thousands of loci to human diseases, yet the vast majority of associated variants fall in noncoding regions where no existing method could predict their functional effects [@maurano_noncode_2012]. The field had accumulated catalogs of where regulatory elements existed; it had no principled way to predict how sequence changes would alter what those elements do.

This asymmetry between annotation abundance and functional prediction defined the central challenge that convolutional neural networks would begin to address. In 2015, a CNN trained on ENCODE chromatin data learned to recognize transcription factor binding motifs matching JASPAR database entries, despite never seeing those motifs during training [@zhou_deepsea_2015]. The network had discovered, through gradient descent on raw sequence, patterns that took decades of experimental work to catalog. The key insight was not merely that deep learning could match human-curated databases, but that learned representations could transcend them: predicting regulatory effects for any sequence in any genomic context, including regions never assayed in any experiment. Every downstream analysis, from polygenic risk scores to clinical variant interpretation, depends on this upstream conversion from sequence to function. When that conversion fails, every subsequent model inherits the failure.

The models examined here, from DeepSEA's chromatin predictions through SpliceAI's clinical-grade splice site detection, established paradigms that persist in modern genomic AI. They also revealed a fundamental architectural limitation. Convolutional networks integrate information only within their **receptive fields**, and genomic regulation routinely operates across distances these models cannot reach. We can learn precise local regulatory grammar, yet we cannot model the long-range interactions that determine which enhancers control which genes. Understanding both what CNNs achieved and where they reached this ceiling establishes the foundation for the attention mechanisms examined in @sec-attention.
...
**Pooling operations** between convolutional layers reduce spatial resolution while increasing the receptive field. Max pooling selects the strongest activation within a window, achieving position-invariant detection where the network responds to a motif's presence somewhere in a region rather than its exact position. This property suits regulatory genomics, where binding site positions within an enhancer often matter less than their presence and combination.
...
Three-panel figure: (A) A single filter sliding across one-hot DNA, producing activation scores; (B) Comparison of learned filter weights to JASPAR motif logo; (C) Receptive field diagram showing how pooling and stacking expands the region influencing each output.
:::
...
**In silico saturation mutagenesis** extends single-variant scoring to systematic characterization. By predicting effects of all possible substitutions across a regulatory element, one identifies positions where mutations most strongly perturb function. These critical positions typically correspond to transcription factor binding motifs, providing motif discovery that emerges from learned representations rather than explicit sequence alignment. The approach enables characterization of any regulatory element, including those in cell types or conditions never experimentally profiled.
...
SpliceAI achieved 95% top-k accuracy for splice site identification (compared to 57% for MaxEntScan) and 0.98 precision-recall AUC. Complex genes exceeding 100 kb are often reconstructed to nucleotide precision. Performance improved dramatically with context length:
...
The limitation matters because genomic regulation routinely operates across distances these models cannot reach. Enhancers regulate promoters 50 to 500 kilobases away. The *beta-globin* locus control region sits 40 to 60 kb from the genes it activates. Polycomb-mediated repression involves chromatin contacts spanning megabases. Topologically associating domains organize regulatory interactions across hundreds of kilobases. When regulatory elements and their targets lie beyond a model's receptive field, the model cannot learn their relationship regardless of how much training data is available. The constraint is architectural, not statistical.
...
## Foundational Principles for Modern Genomic AI

The convolutional models examined here established paradigms that persist in modern genomic AI, even as transformer architectures have become dominant for many applications. These contributions extend beyond specific models to foundational principles that shape how the field approaches sequence-based prediction.

Raw sequence as input, with one-hot encoded nucleotides and no hand-crafted features, demonstrated that end-to-end learning could discover complex regulatory signals without encoding human assumptions about what matters. Position weight matrices required aligned binding site sequences; DeepSEA's filters emerged from gradient descent on chromatin labels. Modern foundation models maintain this commitment to learning from sequence rather than from annotations, and subsequent architectures have only reinforced the value of letting models discover relevant patterns from data.

Multi-task training emerged as essential for learning generalizable representations. Jointly predicting hundreds of related chromatin features improved both accuracy and generalization compared to training separate models for each task. Shared representations in early layers benefit all predictions, and joint training provides implicit regularization against overfitting to any single task. Foundation models extend this principle dramatically, training on thousands of diverse tasks to learn representations that transfer broadly across prediction problems.

Scoring variants through in silico mutagenesis, comparing predictions for reference and alternative sequences, established the dominant paradigm for deep learning-based variant effect prediction. This ab initio capability avoids linkage disequilibrium confounding and enables scoring of rare variants, de novo mutations, and hypothetical sequences never observed in any population. Because models never train on variant data directly, their predictions are unconfounded by the ascertainment biases that affect association-based methods.

SpliceAI demonstrated that task-specific architectures can achieve clinical-grade accuracy that broader foundation models have not yet matched for well-defined problems. When the prediction target is clear and training data are abundant, specialized models remain competitive with or superior to general-purpose approaches. This tension between specialized and general-purpose models recurs throughout subsequent chapters. For clinical applications requiring high accuracy on specific tasks such as splice site prediction, specialized CNNs may remain preferred. For discovery applications requiring broad coverage of molecular mechanisms across diverse prediction targets, the foundation model approach examined in Part IV offers different trade-offs.

The architectures examined in @sec-attention provide mechanisms for long-range dependency modeling that extend the principles established here while transcending the receptive field limitations that constrained these pioneering models.