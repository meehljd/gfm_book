...
|-----------------|------------|---------------|
| **Genomics / Statistical Genetics** | Skim Part I (familiar territory), then Part II (deep learning foundations) | Parts III-IV for foundation models; Part V for evaluation |
| **Machine Learning / Deep Learning** | Part I (genomic data and classical methods) | Parts II-III (sequence architectures); Part V (genomics-specific pitfalls) |
| **Clinical / Translational Research** | Part I (@sec-ch02-data, @sec-ch04-vep-classical) | Part V (evaluation and trust); Part VI (clinical translation) |
...
**Part II: Sequence Architectures** introduces the conceptual and technical foundations of modern sequence modeling. @sec-ch05-representations examines how genomic and protein sequences are converted into model-compatible representations, covering one-hot encodings, k-mers, byte-pair encodings, learned embeddings, and position encodings, showing how these choices shape downstream model behavior. @sec-ch06-cnn examines convolutional approaches that established the field of genomic deep learning, including *DeepSEA*, *Basset*, and *SpliceAI*, analyzing what they learn about motifs and regulatory grammar and where their fixed receptive fields impose limitations that motivate attention-based architectures. @sec-ch07-attention provides a detailed treatment of attention mechanisms, position encodings, and transformer architectures, with emphasis on how these ideas translate from language to biological sequence. @sec-ch08-pretraining covers pretraining objectives, from masked language modeling and next-token prediction to contrastive and generative approaches, examining how self-supervision extracts structure from unlabeled biological data. @sec-ch09-transfer addresses transfer learning, domain adaptation, and few-shot learning, asking when and how pretrained representations generalize to new tasks, species, and data modalities. @sec-ch10-adaptation examines parameter-efficient fine-tuning methods that adapt foundation models to specific tasks. @sec-ch11-benchmarks surveys evaluation benchmarks and methodology, and @sec-ch12-confounding addresses the confounding and leakage issues that pervade genomic evaluation.

**Part III: Foundation Model Families** surveys the major foundation model families, organized by modality, and establishes variant effect prediction as the integrating application. @sec-ch13-fm-principles develops a working definition and taxonomy of foundation models in genomics, distinguishing them from earlier supervised approaches and examining scaling laws that characterize how model capabilities change with size and data. @sec-ch14-dna-lm covers DNA language models such as *DNABERT*, *Nucleotide Transformer*, *HyenaDNA*, and *Evo*, tracing their training corpora, objectives, evaluation suites, and current capabilities. @sec-ch15-protein-lm describes large protein language models trained on evolutionary sequence databases, their emergent structure and function representations, and applications to structure prediction and design. @sec-ch16-regulatory covers hybrid CNN-transformer and related architectures designed for long genomic contexts, such as *Enformer* and *Borzoi*, which predict regulatory readouts over tens to hundreds of kilobases. @sec-ch17-vep-fm serves as a capstone that integrates these model families, examining how protein-based approaches such as *AlphaMissense* and DNA-based approaches such as splicing and regulatory models combine to address variant effect prediction, the central interpretive challenge that motivates the field.

**Part IV: Cellular Context** examines how foundation model principles extend beyond one-dimensional sequence to embrace cellular and systems-level biology. @sec-ch18-rna extends beyond splicing to RNA structure prediction and RNA foundation models, examining how secondary structure and functional context inform representation learning. @sec-ch19-single-cell covers foundation models for single-cell transcriptomics and epigenomics, showing how transformer architectures adapt to the unique characteristics of these data types. @sec-ch20-3d-genome addresses the three-dimensional organization of the genome, from chromatin loops and TAD boundaries to emerging spatial transcriptomics foundation models, examining how 3D structure provides the missing link between sequence and regulatory function. @sec-ch21-networks turns to graph neural networks and network-based approaches, framing these not as alternatives to sequence models but as higher-level reasoning systems that consume foundation model embeddings as node features. @sec-ch22-multi-omics broadens the view to multi-omics integration, exploring how models can jointly represent genomic, transcriptomic, proteomic, and clinical information to connect sequence variation to phenotype across multiple layers of biological organization.

**Part V: Evaluation and Trust** develops frameworks for assessing what models actually learn and how reliably they perform. @sec-ch23-uncertainty addresses uncertainty quantification, examining calibration, epistemic versus aleatoric uncertainty, and practical methods such as ensembles and conformal prediction that help models express when they do not know. @sec-ch24-interpretability explores interpretability tools from classical motif discovery and attribution methods to emerging mechanistic approaches, asking when these tools reveal genuine biological mechanisms and when they provide false comfort. @sec-ch25-causal examines causal inference approaches and their intersection with foundation models. @sec-ch26-regulatory concludes with regulatory and ethical considerations.

**Part VI: Clinical Translation** moves from methods to end-to-end workflows in research and clinical practice. @sec-ch27-clinical-risk discusses clinical risk prediction that combines genomic features with electronic health records and environmental data, focusing on discrimination, calibration, fairness, and deployment in health systems. @sec-ch28-rare-disease examines how foundation models fit into rare disease and cancer workflows, including variant prioritization pipelines, integration with family and tumor-normal data, and laboratory validation. @sec-ch29-drug-discovery looks at how GFMs intersect with target discovery, functional genomics screens, and biomarker development in pharmaceutical and biotechnology settings. @sec-ch30-design covers generative applications, from protein design and therapeutic sequence optimization to synthetic biology and bioengineering workflows. @sec-ch31-frontiers examines emerging directions and open problems.
