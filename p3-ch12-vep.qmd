...
While zero-shot scores are already competitive, Evo 2 can be fine-tuned or paired with task-specific heads for regulatory prediction (e.g., chromatin accessibility, histone marks), splicing outcomes, and gene expression levels. Such adapters reuse Evo 2's representations as a generic backbone, mirroring how text LLMs are adapted via lightweight fine-tuning and prompting (@sec-princ).
...
In contrast, models like AlphaGenome and Evo 2 are generalist backbones that can be adapted across many contexts via multi-task heads or fine-tuning. Generalist models provide a single representation that can be reused across tasks (@sec-princ), enable transfer to cell types or assays with limited data, and support unified analyses that integrate multiple regulatory modalities.
