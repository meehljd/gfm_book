# Single-Cell and Epigenomic Models {#sec-single-cell}
...
The three-dimensional organization of the genome, examined in @sec-3d-genome, provides crucial context for interpreting cellular state. Chromatin accessibility and gene expression are not independent of genome folding; enhancer-promoter contacts determine which regulatory elements can influence which genes. Models that integrate 3D structure with cellular state measurements may achieve more accurate predictions than either approach alone.

Spatial transcriptomics (@sec-spatial) adds positional context that reveals how cellular state varies across tissue architecture, connecting expression profiles to physical location.

Cell embeddings from foundation models can serve as node features in graph-based reasoning systems (see @sec-networks). Protein-protein interactions, regulatory networks, and pathway databases provide relational structure that pure sequence models cannot capture. Using foundation model embeddings as inputs to graph neural networks combines the representational power of large-scale pretraining with the relational inductive biases of graph architectures.

Multi-omics integration (see @sec-multi-omics) extends beyond the two-modality cases examined here to encompass proteomics, metabolomics, spatial transcriptomics, and clinical data. The GLUE framework demonstrates how biological prior knowledge can guide integration across modalities with different feature spaces. Scaling these approaches to many modalities while maintaining computational tractability remains an active research direction.
