...
**Prerequisites:** This chapter builds on concepts from @sec-ch12-confounding (data leakage and validation pitfalls), @sec-ch23-uncertainty (uncertainty quantification), and @sec-ch24-attribution (interpretability methods). Familiarity with variant effect prediction (@sec-ch14-protein-vep) and clinical genomics applications (@sec-ch27-clinical) will enhance understanding of regulatory context.
...
![SaMD risk classification for genomic AI](../figs/part_5/ch26/03-fig-samd-classification.svg)
...
The chapter on confounding (@sec-ch12-confounding) introduced a critical validation pitfall. Can you recall what it is? How might this same pitfall affect regulatory submissions for a variant classifier trained on ClinVar data?
...
Clinical validation connects model outputs to clinical outcomes. For a variant classifier, clinical validation might assess concordance with expert panel classifications, correlation with functional assay results, or agreement with segregation patterns in affected families. The choice of reference standard is itself contentious: ClinVar classifications, which many models use as training labels, reflect historical expert consensus that may lag behind accumulating evidence, and circular validation using the same database for training and evaluation produces misleadingly optimistic results (see @sec-ch12-confounding). The deployment realities discussed in @sec-ch27-validation-hierarchy and @sec-apx-b-compute illustrate how these validation requirements interact with institutional workflows, reimbursement constraints, and clinician trust; regulatory clearance represents only one barrier among many.
...
The validation requirements you see here—prospective evaluation, ancestry stratification, comparison to existing tools—directly shape the clinical deployment workflows covered in @sec-ch27-clinical-risk and @sec-ch28-rare-disease. A model that clears regulatory hurdles with aggregate metrics but lacks ancestry-stratified validation will fail the equity requirements discussed in @sec-ch12-confounding. The evidence pyramid you'll encounter in @sec-ch28-acmg-evidence depends on regulatory-grade validation to assign computational evidence strength.
:::
...
::: {#fig-consent-models}
![Consent model spectrum for genomic data](../figs/part_5/ch26/04-fig-consent-models.svg)
...
![Privacy-utility tradeoff: stronger protection reduces utility](../figs/part_5/ch26/01-A-fig-data-governance.svg)

![Consent models from broad to dynamic](../figs/part_5/ch26/01-B-fig-data-governance.svg)

![Federated learning: data stays local, model travels](../figs/part_5/ch26/01-C-fig-data-governance.svg)

![Cross-border regulatory complexity](../figs/part_5/ch26/01-D-fig-data-governance.svg)
...
::: {#fig-liability-chain}
![Liability chain for AI-informed clinical decisions](../figs/part_5/ch26/05-fig-liability-chain.svg)
...
Effective oversight requires that model outputs be interpretable enough for humans to exercise meaningful judgment. If a variant classifier provides only a pathogenic/benign label without supporting evidence, the overseeing clinician has no basis for assessing whether the model's reasoning applies to the case at hand. If a regulatory effect predictor reports a large effect without indicating uncertainty, the user may not know when skepticism is warranted. Interpretability tools discussed in @sec-ch24-attribution and @sec-ch24-mechanistic support oversight by revealing internal model reasoning, but interpreting such explanations requires expertise and time that may not be available in clinical workflows.
...
![Dual-use risk assessment for generative genomic models](../figs/part_5/ch26/02-fig-dual-use-governance.svg)
...
- **Clinical risk prediction** (@sec-ch27-clinical): Polygenic risk scores face equity challenges when derived from ancestry-biased training data, and clinical deployment requires navigating the SaMD framework
- **Rare disease diagnosis** (@sec-ch28-rare-disease): Consent challenges arise when patient data contributes to models that benefit future patients; the liability chain from model developer to clinician affects diagnostic workflows
- **Drug discovery** (@sec-ch29-drug): Dual-use concerns intensify when generative models design novel therapeutic candidates that could theoretically be misused
- **Sequence design** (@sec-ch30-design): Regulatory requirements for synthetic biology products intersect with AI governance when foundation models guide design
- **Frontiers** (@sec-ch31-frontiers): Translation barriers include not just technical challenges but regulatory bottlenecks that slow clinical deployment
...
- **Backward:** Builds on confounding (@sec-ch12-confounding), uncertainty (@sec-ch23-uncertainty), and interpretability (@sec-ch24-attribution) as technical foundations for responsible deployment
- **Forward:** Regulatory principles recur in clinical risk (@sec-ch27-clinical), rare disease (@sec-ch28-rare-disease), drug discovery (@sec-ch29-drug), and sequence design (@sec-ch30-design); translation barriers in @sec-ch31-frontiers include regulatory bottlenecks
