...
**Prerequisites:** This chapter builds on concepts from earlier chapters including model calibration basics (@sec-ch11-eval), pretraining objectives (@sec-ch08-pretraining), and variant effect prediction (@sec-ch17-vep-fm). Familiarity with basic probability and statistics (means, variances, distributions) is assumed.
...
![Aleatoric: irreducible stochastic variability](../figs/part_5/ch23/01-A-fig-uncertainty-types.svg)

![Epistemic: reducible knowledge-based uncertainty](../figs/part_5/ch23/01-B-fig-uncertainty-types.svg)
...
**Calibration** determines whether model outputs can be interpreted as probabilities. A score of 0.85 from a pathogenicity predictor should mean that 85% of variants receiving similar scores are truly pathogenic; only then can clinicians rationally weight computational evidence against other diagnostic criteria. This chapter provides the comprehensive treatment of calibration theory and methods. Readers encountering calibration in applied contexts, whether variant effect prediction (@sec-ch14-calibration) or evaluation methodology (@sec-ch11-eval), are referred here for the formal foundations and complete methodological catalog.

::: {#fig-calibration-diagrams layout-ncol=2}
![Perfect calibration: predictions match observed frequencies](../figs/part_5/ch23/02-A-fig-calibration-diagrams.svg)

![Overconfident: predictions exceed reality](../figs/part_5/ch23/02-B-fig-calibration-diagrams.svg)

![Underconfident: predictions underestimate true probabilities](../figs/part_5/ch23/02-C-fig-calibration-diagrams.svg)

![Calibration metrics: ECE and MCE quantify miscalibration](../figs/part_5/ch23/02-D-fig-calibration-diagrams.svg)
...
![Temperature scaling: single parameter, preserves ranking](../figs/part_5/ch23/03-A-fig-calibration-methods.svg)

![Platt scaling: handles asymmetric miscalibration](../figs/part_5/ch23/03-B-fig-calibration-methods.svg)

![Isotonic regression: flexible non-parametric fit](../figs/part_5/ch23/03-C-fig-calibration-methods.svg)
...
For zero-shot variant effect scores like *ESM-1v* log-likelihood ratios, raw outputs have no inherent probabilistic interpretation. Calibration requires mapping these continuous scores to pathogenicity probabilities using external labels, typically from ClinVar or population frequency data. This mapping should occur on held-out genes or variants not used for any model development, and the resulting calibration reflects the specific label set used; calibration against ClinVar pathogenic/benign labels may not transfer to other clinical contexts. The principles of proper held-out evaluation are discussed in @sec-ch11-eval.
...
::: {#fig-uq-methods}
![Uncertainty quantification methods overview](../figs/part_5/ch23/04-fig-uq-methods.svg)
...
![Conformal prediction provides coverage-guaranteed prediction sets](../figs/part_5/ch23/05-fig-conformal-prediction.svg)
...
![The OOD problem: models extrapolate confidently into unreliable regions](../figs/part_5/ch23/06-A-fig-ood-detection.svg)

![Detection methods: from softmax thresholds to embedding approaches](../figs/part_5/ch23/06-B-fig-ood-detection.svg)

![Genomic OOD scenarios requiring detection before clinical use](../figs/part_5/ch23/06-C-fig-ood-detection.svg)
...
![Selective prediction enables tiered clinical workflows](../figs/part_5/ch23/07-fig-selective-prediction.svg)
...
- Builds on evaluation principles (@sec-ch11-eval) and pretraining objectives (@sec-ch08-pretraining)
- Precedes interpretability (@sec-ch24-interpretability) which explains why models are uncertain
- Applies to clinical risk prediction (@sec-ch27-clinical-risk) and rare disease diagnosis (@sec-ch28-rare-disease)
- Intersects with fairness concerns (@sec-ch03-fairness, @sec-ch26-regulatory)
:::
