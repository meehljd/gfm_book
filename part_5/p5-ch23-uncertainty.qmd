...
## Types of Uncertainty in Genomic Prediction {#sec-ch23-types-uncertainty-genomic-prediction}
...
## Post-Hoc Calibration Methods {#sec-ch23-post-hoc-calibration-methods}
...
## Uncertainty Quantification Methods for Foundation Models {#sec-ch23-uncertainty-quantification-methods}
...
## Conformal Prediction: Distribution-Free Guarantees {#sec-ch23-conformal-prediction-distribution-free}
...
For regression tasks such as expression prediction or quantitative trait prediction, conformalized quantile regression produces intervals guaranteed to contain the true value with specified probability. The method fits separate models for the lower and upper quantiles of the conditional distribution, then applies conformal calibration to ensure finite-sample coverage.

The resulting prediction intervals are directly interpretable: a 90% prediction interval for gene expression change contains the true change at least 90% of the time. Unlike Bayesian credible intervals, which depend on prior assumptions, conformal intervals provide frequentist coverage guarantees that hold regardless of the model's correctness.

Interval width varies with prediction difficulty. Variants where the model is confident produce narrow intervals; variants where the model is uncertain produce wide intervals. This adaptive behavior makes conformal intervals useful for identifying predictions that warrant additional scrutiny or experimental validation.
...
## Out-of-Distribution Detection {#sec-ch23-out-distribution-detection}
...
## Selective Prediction and Abstention {#sec-ch23-selective-prediction-abstention}
...
## Uncertainty for Specific Genomic Tasks {#sec-ch23-uncertainty-specific-genomic-tasks}
...
## Communicating Uncertainty to End Users {#sec-ch23-communicating-uncertainty-end-users}
