...
> Same genome, different cells. Single-cell models capture how cells interpret their DNA.
...
![Language models predict masked words from context](../figs/part_5/ch20/01-A-fig-cellular-lm-analogy.svg)

![Cellular LMs predict masked genes from expression context](../figs/part_5/ch20/01-B-fig-cellular-lm-analogy.svg)

![Parallel structure between NLP and single-cell domains](../figs/part_5/ch20/01-C-fig-cellular-lm-analogy.svg)

![Comparison of what each model type learns](../figs/part_5/ch20/01-D-fig-cellular-lm-analogy.svg)
...
Batch effects arise because technical variation between experiments often exceeds biological variation within them. Cells processed on different days, by different operators, or with different reagent lots may cluster by batch rather than by biological type. A model that learns batch-specific patterns rather than biological ones will fail to generalize. This challenge parallels the confounding issues examined in @sec-ch13-confounding, where technical artifacts can masquerade as biological signal.
...
![Dropout and sparsity: over 90% zeros in gene-cell matrices](../figs/part_5/ch20/02-A-fig-single-cell-challenges.svg)

![Batch effects can exceed biological variation](../figs/part_5/ch20/02-B-fig-single-cell-challenges.svg)

![Dynamic range spans orders of magnitude](../figs/part_5/ch20/02-C-fig-single-cell-challenges.svg)

![Foundation model strategies for addressing challenges](../figs/part_5/ch20/02-D-fig-single-cell-challenges.svg)
...
The analogy between gene expression and language has proven highly productive. If cells are sentences and genes are words, then cellular regulatory programs are grammar: the rules governing which genes appear together in which contexts. Several foundation models now operationalize this analogy, treating single-cell transcriptomes as documents and learning to predict masked genes from expression context. These models differ in their tokenization strategies, pretraining objectives, and architectural choices, but share a common premise: that self-supervised learning on millions of cells can capture regulatory logic that transfers to diverse downstream tasks.
...
4. You are investigating whether a regulatory network identified in mouse immune cells is conserved in human.
...
3. Attention weights indicate which genes the model considers together during prediction. These patterns often correlate with known regulatory interactions (transcription factors and their targets), though whether this reflects genuine regulatory biology or statistical co-occurrence requires further validation.
:::
:::

::: {#fig-geneformer-architecture layout-ncol=2}
![Rank-based encoding highlights unusually active genes](../figs/part_5/ch20/03-A-fig-geneformer-architecture.svg)

![Transformer encoder architecture with masked gene prediction](../figs/part_5/ch20/03-B-fig-geneformer-architecture.svg)

![Attention weights correspond to regulatory relationships](../figs/part_5/ch20/03-C-fig-geneformer-architecture.svg)

![Transfer to cell annotation and target identification](../figs/part_5/ch20/03-D-fig-geneformer-architecture.svg)
...
In zero-shot settings, *TranscriptFormer* demonstrates strong performance on both in-distribution and out-of-distribution cell type classification. Strikingly, models trained predominantly on mouse and human data can annotate cell types in zebrafish and other species separated by hundreds of millions of years of evolution. This cross-species transfer reveals that core principles of cellular regulation are deeply conserved, and that foundation models can capture these conserved principles when trained on evolutionarily diverse data. The success of cross-species transfer in cellular models parallels similar findings in protein language models, where evolutionary conservation provides a powerful inductive bias (see @sec-ch16-protein-lm).
...
This conservation is precisely why self-supervised learning on expression data works: the "grammar" of cellular regulation is shared across species, just as the grammar of regulatory sequences is shared across the tree of life (see @sec-ch15-gpn for parallel cross-species insights in DNA models).
:::

## Perturbation Response Prediction {#sec-ch20-perturbation}

The ultimate test of whether cellular foundation models understand regulatory biology is prediction: can they anticipate how cells will respond to interventions they have never seen? Perturbation prediction moves beyond pattern recognition toward mechanistic understanding. If a model has learned the causal structure of gene regulatory networks, it should predict the downstream consequences of knocking out a transcription factor or activating a signaling pathway. This capability would transform drug discovery and target identification (@sec-ch30-drug-discovery), enabling *in silico* screening of perturbations before expensive wet-lab validation. The design-build-test-learn cycles that could exploit such predictions are examined in @sec-ch31-dbtl. Achieving this capability requires models to distinguish causation from correlation in observational data.
...
When evaluating these foundation model approaches, comparison against classical methods remains essential. SCENIC [@aibar_scenic_2017] provides a widely-used baseline for gene regulatory network inference from single-cell expression data, using cis-regulatory sequence analysis to identify putative regulons. Foundation models claiming improved perturbation prediction should demonstrate improvement over SCENIC-derived baselines to establish genuine advances beyond classical co-expression approaches.

::: {#fig-perturbation-prediction layout-ncol=2}
![Predict post-perturbation expression from cell state and perturbation](../figs/part_5/ch20/04-A-fig-perturbation-prediction.svg)

![Training data from Perturb-seq provides supervised signal](../figs/part_5/ch20/04-B-fig-perturbation-prediction.svg)

![Models must learn directional relationships and cascades](../figs/part_5/ch20/04-C-fig-perturbation-prediction.svg)

![Performance correlates with gene characterization level](../figs/part_5/ch20/04-D-fig-perturbation-prediction.svg)
...
- You are predicting combinatorial effects
...
![Unpaired integration: different modalities in different cells](../figs/part_5/ch20/05-A-fig-glue-architecture.svg)

![VAE encoders feed into shared latent space](../figs/part_5/ch20/05-B-fig-glue-architecture.svg)

![Feature graph encodes biological prior knowledge](../figs/part_5/ch20/05-C-fig-glue-architecture.svg)

![Applications include cross-modal prediction and regulatory inference](../figs/part_5/ch20/05-D-fig-glue-architecture.svg)
...
Single-cell and epigenomic foundation models learn what states cells occupy, complementing the sequence-based models that learn what sequences encode. DNA and protein language models capture the information content of genomic and protein sequence (see @sec-ch15-dna-lm, @sec-ch16-protein-lm); cellular models capture the configurations that cells assume in development, homeostasis, and disease. These perspectives address different biological questions: sequence determines the possible states a cell can achieve, while cellular state reflects which possibilities are realized in a given context. A complete understanding of gene regulation requires both.

The representations learned by cellular foundation models enable integration across scales and modalities. Cell embeddings serve as node features in graph-based reasoning systems (@sec-ch22-networks), connecting expression profiles to protein interaction networks and regulatory pathways. Three-dimensional genome organization (@sec-ch21-3d-genome) provides spatial context that constrains which regulatory relationships can operate. Multi-omics integration (@sec-ch23-multi-omics) extends beyond expression to proteomics, epigenomics, and clinical measurements. In each case, foundation model embeddings provide the representational substrate that downstream methods refine.
...
- *Forward:* Cell embeddings feed into graph-based reasoning (@sec-ch22-networks) and multi-omics integration (@sec-ch23-multi-omics)
- *Forward:* Perturbation prediction enables drug discovery workflows (@sec-ch30-drug-discovery)
...
- *Backward:* Technical confounding mirrors batch effect challenges in @sec-ch13-confounding
:::
