...
**Prerequisites:** This chapter builds on convolutional neural network architectures (@sec-ch06-cnn), attention mechanisms (@sec-ch07-attention), DNA language models (@sec-ch14-dna-lm), and protein language models (@sec-ch15-protein-lm). Familiarity with gradient-based optimization and basic linear algebra will help with the mathematical sections.
...
![Attribution methods comparison on the same genomic sequence](../figs/part_5/ch24/01-fig-attribution-comparison.svg)
...
![Procedure: substitute each position to all alternatives](../figs/part_5/ch24/02-A-fig-in-silico-mutagenesis.svg)

![Visualization: position × mutation heatmap](../figs/part_5/ch24/02-B-fig-in-silico-mutagenesis.svg)

![Validation against deep mutational scanning](../figs/part_5/ch24/02-C-fig-in-silico-mutagenesis.svg)

![Mechanistic insights from ISM profiles](../figs/part_5/ch24/02-D-fig-in-silico-mutagenesis.svg)
...
::: {#fig-tfmodisco}
![TF-MoDISco motif discovery from attribution scores](../figs/part_5/ch24/03-fig-tfmodisco.svg)
...
![Attention heatmap showing position-position weights](../figs/part_5/ch24/04-A-fig-attention-visualization.svg)

![Biological overlay: attention sometimes aligns with regulatory elements](../figs/part_5/ch24/04-B-fig-attention-visualization.svg)

![Multi-head specialization: different heads capture different patterns](../figs/part_5/ch24/04-C-fig-attention-visualization.svg)
...
In models like *Enformer* that predict regulatory outputs from long genomic windows (see @sec-ch16-enformer), attention can reveal which distal regions influence predictions at a target gene. Contribution scores aggregated across attention heads often peak at known enhancers, insulators, and chromatin domain boundaries. These patterns suggest that the model has learned aspects of regulatory architecture from the correlation between sequence and chromatin output labels.
...
![Plausible versus faithful explanations: validation distinguishes them](../figs/part_5/ch24/05-fig-plausible-vs-faithful.svg)
...
![Closed-loop interpretability validation workflow](../figs/part_5/ch24/06-fig-validation-pipeline.svg)
...
Variant interpretation guidelines require that computational predictions be weighed alongside experimental and clinical evidence, as discussed further in @sec-ch28-rare-disease. Interpretability determines whether model predictions can contribute meaningful evidence beyond raw pathogenicity scores.
...
Foundation model predictions are less immediately interpretable but potentially more informative. A pathogenicity score from *ESM-1v* (@sec-ch15-esm-family) reflects evolutionary constraint inferred from protein language modeling, but the specific sequence features driving the prediction require attribution analysis to identify. The protein VEP paradigm is examined in @sec-ch14-protein-vep. An expression effect predicted by *Enformer* (@sec-ch16-enformer) might result from disrupted transcription factor binding, altered chromatin accessibility, or changed 3D regulatory contacts; interpretability analysis distinguishes these mechanisms and guides experimental validation. The DNA-based VEP approaches are detailed in @sec-ch14-dna-vep.
...
![Interpretability for clinical variant assessment](../figs/part_5/ch24/07-fig-clinical-interpretability.svg)
...
- Interpretability enables clinical utility by providing mechanistic evidence that satisfies ACMG-AMP criteria (@sec-ch26-acmg-amp, @sec-ch28-rare-disease)
- Confounder detection (@sec-ch12-confounding) relies on interpretability to identify shortcuts
- Sequence design (@sec-ch30-design) uses interpretability-derived hypotheses to guide experimental validation

**Looking Ahead:** @sec-ch25-causal extends interpretability to causal inference, examining how to distinguish correlation from causation in model predictions and when interpretable features reflect genuine regulatory mechanisms.
:::
