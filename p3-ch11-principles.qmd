...
::: {.callout-note}
**Figure 11.1 suggestion:** A 2×2 taxonomy grid showing the four families of genomic foundation models. The horizontal axis could represent "sequence-centric" versus "variant-centric" approaches, while the vertical axis represents "single-modality" versus "multi-modal" integration. Each quadrant would contain representative models: DNA language models (DNABERT, Nucleotide Transformer, HyenaDNA, GROVER), sequence-to-function models (Enformer, Sei, DeepSEA), variant-centric models (AlphaMissense, CADD, DELFI/DELpHI, MIFM), and multi-omic models (Omni-DNA, cross-modal architectures).
:::
...
::: {.callout-note}
**Table 11.1 suggestion:** A comparison table of representative genomic foundation model families with columns for: Model Family, Example Models, Context Length (typical range), Parameter Count (order of magnitude), Pretraining Objective, Primary Readouts, and Typical Downstream Applications. This would help readers quickly compare trade-offs across the taxonomy.
:::
...
::: {.callout-note}
**Figure 11.2 suggestion:** A schematic diagram showing four orthogonal axes representing the design dimensions: Data (species coverage, assay diversity, population diversity), Architecture (CNN/transformer/Hyena, attention mechanisms), Objectives (MLM, autoregressive, multi-task), and Tokenization/Representations (character-level, k-mer, learned BPE). Each axis should be annotated with concrete choices to illustrate the design space.
:::
...
A recent review of transformers in genomics explicitly argues that MLM and autoregressive objectives were imported from NLP with little biological tailoring, and may not be the right pretraining objectives for genomics [@consens_transformers_2023]. The authors point to results showing random k-mer embeddings can perform comparably to biologically pre-trained embeddings on some downstream tasks, questioning how much genomic grammar current pretraining actually learns. This critique suggests that the field should explore more biologically motivated pretext tasks, such as Self-GenomeNet's use of reverse complements and neighboring sequence structure, rather than reflexively adopting NLP objectives.
...
::: {.callout-note}
**Figure 11.3 suggestion:** An evaluation pyramid diagram with four levels. The base represents token-level or base-pair tasks (e.g., masked prediction, motif recognition). The second level shows molecular readouts (chromatin accessibility, TF binding, gene expression). The third level represents cellular and tissue phenotypes (cell state, differentiation, response to perturbation). The apex represents organismal and clinical outcomes (disease risk, drug response, fitness). Arrows should indicate that validation evidence should ideally accumulate from lower to higher levels.
:::
...
::: {.callout-note}
**Figure 11.4 suggestion:** A decision tree flowchart for model selection. Starting from "What is your primary input?" (variants, sequences, loci), branch by prediction target (molecular readout, trait association, pathogenicity), then by available data (labeled functional data, reference genomes only), and finally by computational constraints (high-throughput scoring, exploratory analysis). Terminal nodes would recommend appropriate model families with brief justifications.
:::
...
::: {.callout-note}
**Figure 11.5 suggestion:** A comparison diagram of adaptation strategies showing four approaches: (1) frozen backbone with linear probe, (2) frozen backbone with shallow MLP, (3) frozen backbone with LoRA adapters, and (4) full fine-tuning. For each, annotate approximate parameter counts (both trainable and total), memory requirements, and typical compute requirements. Include rough guidance on when each approach is appropriate based on available labeled data and computational resources.
:::
...
Work on large biological graph models suggests potential mitigation strategies. Ma and colleagues applied adversarial perturbations to internal GNN embeddings (not raw sequence) and observed small performance gains but noticeably smoother and lower training loss, suggesting reduced overfitting [@ma_bingo-large_2023]. Similar adversarial training approaches may help stabilize genomic foundation models, though this remains an active area of investigation.
