...
Each molecular layer captures part of the biological story but not all of it. Genomic variants identify predisposition; transcriptomics reveals which genes respond; proteomics shows which proteins change; metabolomics measures downstream biochemical consequences. A patient with a BRCA1 variant (genomic) may show altered DNA repair gene expression (transcriptomic), deficient homologous recombination protein activity (proteomic), and characteristic metabolic signatures (metabolomic). No single layer provides the complete picture. Effective integration traces this causal cascade from genetic variation through molecular intermediates to clinical phenotype, distinguishing primary effects from downstream consequences and noise from signal.

This chapter examines strategies for integrating multiple data types into unified representations that capture more than any single modality alone. We compare early fusion (concatenating features before modeling), intermediate fusion (learning joint representations), and late fusion (combining predictions from modality-specific models), analyzing tradeoffs for different applications and data characteristics. We survey multi-omics foundation models that learn across genomics, transcriptomics, proteomics, and other modalities simultaneously. We extend to clinical integration, where electronic health records, imaging data, and molecular measurements must be combined for patient-level prediction. Throughout, we address the practical challenges: missing modalities (not every patient has every assay), batch effects (technical variation between measurement platforms), and the gap between multi-omics potential and deployment reality.

## The Limits of Single-Modality Models
...
## The Systems View: From Variant to Phenotype
