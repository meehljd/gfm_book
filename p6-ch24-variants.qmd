...
### Variant Effect Prediction in Clinical Context

Variant effect prediction models provide computational estimates of whether a genetic variant is likely to be pathogenic, benign, or functionally consequential. The landscape of VEP methods ranges from conservation-based scores through deep learning approaches to foundation models, with detailed treatment in @sec-vep. In clinical practice, these predictions serve as one line of evidence within structured variant interpretation frameworks like ACMG/AMP guidelines, never as standalone diagnostic tools.

The key question for clinical workflows is not "which predictor has the highest AUROC" but rather "how should computational predictions be integrated with other evidence sources?" This includes population frequency data, segregation analysis in families, functional assays, clinical phenotype match, and expert judgment. VEP scores become most useful when properly calibrated to provide interpretable probabilities rather than arbitrary scores, when uncertainty is quantified and communicated clearly, and when limitations (gene coverage, variant type specificity, ancestry biases) are explicitly acknowledged.

## From Variant Effect Prediction to Prioritization

Variant effect predictors (VEPs) map a candidate variant to one or more predicted consequences: changes in transcription factor binding, chromatin accessibility, RNA abundance, splicing, protein stability, and so on (@sec-vep). GFMs greatly expand the coverage and resolution of these predictions. AlphaMissense provides proteome-wide missense effect estimates [@cheng_alphamissense_2023], while long-range regulatory models like Enformer and Borzoi predict complex cis-regulatory outputs from sequence alone [@avsec_enformer_2021; @linder_borzoi_2025]. The architectures, training approaches, and evaluation of these models are detailed in @sec-vep.
