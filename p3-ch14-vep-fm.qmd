# Variant Effect Prediction with Foundation Models {#sec-vep-fm}
...
The chapters that follow examine how variant effect predictions integrate into clinical workflows for rare disease diagnosis (@sec-rare-disease) and risk prediction (@sec-clinical-risk), the broader challenges of uncertainty quantification that apply across genomic applications (@sec-uncertainty), and the interpretability methods that help us understand what foundation models have learned (@sec-interpretability). Variant effect prediction sits at the center of genomic medicine; foundation models have raised its ceiling while leaving substantial work to achieve its potential.