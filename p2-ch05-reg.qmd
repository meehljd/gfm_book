...
ExPecto, published in 2018, included an updated chromatin prediction model nicknamed "Beluga" [@zhou_expecto_2018]. Key improvements included:
...
- **Integration with expression prediction**: Chromatin predictions serve as intermediate features for tissue-specific expression prediction (@sec-trans)
...
However, the original DeepSEA architecture's limited receptive field (due to pooling operations) constrained its ability to learn long-range dependencies. This limitation motivated later architectures with expanded context windows (Enformer, @sec-hybrid).
