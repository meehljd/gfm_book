...
This chapter addresses how to use benchmarks appropriately to draw valid conclusions about model performance. @sec-benchmarks catalogs what benchmark tasks exist, how they are constructed, and what capabilities they probe. Here we address the complementary question: given a benchmark, how do we apply it to produce trustworthy results? The difference between valid and misleading evaluation often lies not in benchmark choice but in methodological details: data splitting strategies, metric selection, baseline comparisons, ablation designs, and statistical testing. These principles apply across all benchmark categories, from chromatin state prediction to clinical variant classification. By mastering evaluation methodology, practitioners can distinguish genuine advances from artifacts that will not survive deployment.
...
Rigorous evaluation requires sustained effort at every stage, from experimental design through statistical analysis. The chapters that follow address complementary aspects of this challenge. Confounding and leakage (@sec-confounding) examines how population stratification, batch effects, and ascertainment bias produce results that evaporate under deployment. Uncertainty quantification (@sec-uncertainty) extends calibration assessment to epistemic versus aleatoric uncertainty and selective prediction. Interpretability (@sec-interpretability) addresses whether models have learned genuine biology or exploited confounded patterns. For clinical applications specifically, risk prediction frameworks (@sec-clinical-risk) develop evaluation approaches tailored to decision-making, where net benefit and decision curves supplement discrimination metrics. Together, these chapters provide the critical apparatus for engaging with genomic foundation model claims.