...
- **DeepSEA**: predicting chromatin marks and transcription factor binding from sequence [@zhou_deepsea_2015].
- **ExPecto**: extending DeepSEA-style CNNs into a modular system that predicts gene expression across hundreds of tissues [@zhou_expecto_2018].
- **SpliceAI**: using very deep, dilated convolutional architectures for splice site prediction [@jaganathan_spliceai_2019].

Though DeepVariant (@sec-deepvar) demonstrated that CNNs could outperform hand-crafted pipelines for variant calling by treating read pileups as images [@poplin_deepvariant_2018], the models examined here solve a fundamentally different problem. Variant calling asks "is this a true variant?" while effect prediction asks "what does this variant do?" Yet they share a common lesson: learned representations can replace the hand-crafted features and heuristics that dominated earlier computational genomics.

At a high level, the CNN sequence-to-function lineage established four key ideas:

1. **Raw sequence as input**: one-hot encoded nucleotides are sufficient to learn complex regulatory signals.
2. **Multi-task training**: jointly predicting hundreds or thousands of related assays improves both accuracy and generalization.
3. **Variant scoring via in silico perturbation**: differences in model predictions under allele changes approximate variant effects.
4. **Task specialization**: models tailored to narrow problem domains can achieve high performance and clinical impact, even without being "foundation models."
...
Timeline or schematic showing DeepSEA (regulation), ExPecto (chromatin → expression), and SpliceAI (splicing) along a horizontal axis, with key innovations (multi-task learning, modular modeling, dilated convolutions, long-range context) annotated.
:::
...
The vast majority of disease-associated variants identified by GWAS lie in noncoding regions of the genome [@maurano_noncode_2012]. Across thousands of loci mapped to complex traits, only a small minority directly alter protein-coding sequences; the remainder fall in introns, intergenic regions, and putative regulatory elements where their functional consequences are far less obvious. This presents both an interpretive challenge and an opportunity. If we could predict how noncoding variants affect gene regulation, we would have a powerful tool for moving from statistical association to biological mechanism.
...
(1) Scatter plot of predicted vs. observed expression for a representative tissue; (2) bar chart of per-tissue correlation; (3) example delta-expression scores for variants near a gene with multiple nearby regulatory elements.
:::
