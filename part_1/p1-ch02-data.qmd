...
Critical literacy about data provenance is prerequisite to interpreting what models learn. Understanding what each resource measures, how it was constructed, and where coverage breaks down enables recognition of when training data may not represent the population, condition, or variant class at hand. The goal is not encyclopedic completeness but critical literacy: the ability to recognize when a model's training data may not represent the population, condition, or variant class at hand. This literacy becomes essential when deploying models in clinical contexts where failures have consequences.


## Reference Genomes and Gene Annotations {#sec-ch02-reference-genomes-gene-annotations}
...
## Population Variant Catalogs and Allele Frequencies {#sec-ch02-population-variant-catalogs-allele}
...
### dbSNP and Variant Identifiers {#sec-ch02-dbsnp-variant-identifiers}
...
### 1000 Genomes and Early Reference Panels {#sec-ch02-1000-genomes-early-reference-panels}
...
## Biobanks and GWAS Data {#sec-ch02-biobanks-gwas-data}
...
### GWAS Summary Statistics {#sec-ch02-gwas-summary-statistics}
...
## Functional Genomics and Regulatory Landscapes {#sec-ch02-functional-genomics-regulatory}
...
### ENCODE, Roadmap, and Related Consortia {#sec-ch02-encode-roadmap-related-consortia}
...
### Cistrome Data Browser {#sec-ch02-cistrome-data-browser}
...
### From Assays to Training Labels {#sec-ch02-assays-training-labels}
...
### Deep Mutational Scanning and Multiplexed Variant Assays {#sec-ch02-deep-mutational-scanning-multiplexed}
...
### Bulk Expression Atlases {#sec-ch02-bulk-expression-atlases}
...
### Single-Cell and Context-Specific Expression {#sec-ch02-single-cell-context-specific-expression}
...
These technologies introduce computational challenges: sparsity from dropout effects, batch variation across samples and technologies, and massive scale with millions of cells per study. They also offer an increasingly fine-grained view of the link between genotype, regulatory state, and cellular phenotype. Multi-omics integration (@sec-multi-omics) and systems-level modeling draw heavily on single-cell and spatial resources.


## Phenotype Definition and Data Quality {#sec-ch02-phenotype-definition-data-quality}
...
### Problem of Binary Disease Definitions {#sec-ch02-problem-binary-disease-definitions}
...
### Electronic Health Record Quality and Completeness {#sec-ch02-electronic-health-record-quality}
...
### Coding Inconsistencies and Label Noise {#sec-ch02-coding-inconsistencies-label-noise}
...
### Deep Phenotyping Approaches {#sec-ch02-deep-phenotyping-approaches}
...
### Impact on Downstream Modeling {#sec-ch02-impact-downstream-modeling}

Phenotype quality constraints propagate through every analysis built on biobank data. Polygenic scores trained on noisy phenotypes learn to predict the noise alongside the signal, potentially inheriting coding artifacts, temporal discontinuities, and population-specific documentation practices. Transfer learning from one biobank to another may fail not because the underlying genetic architecture differs but because the phenotype definitions differ in ways that alter what the model learned.

Foundation models face analogous challenges. A model that learns associations between genetic variants and EHR-derived phenotypes absorbs whatever systematic distortions those phenotypes contain. If a diagnosis is more likely to be coded in patients who receive specialist care, the model learns a genetic signature for healthcare access as much as for disease biology. If a biomarker is measured only in symptomatic patients, the model learns from a biased sample that may not represent the population distribution.
...
## Variant Interpretation Databases and Clinical Labels {#sec-ch02-variant-interpretation-databases}
...
### ClinVar and Clinical Assertions {#sec-ch02-clinvar-clinical-assertions}
...
### Complementary Clinical Databases {#sec-ch02-complementary-clinical-databases}
...
### ClinGen and Expert Curation {#sec-ch02-clingen-expert-curation}
...
## Inherited Constraints {#sec-ch02-inherited-constraints}
...
The critical question is not whether models trained on these data contain biases; they do. The question is whether those biases can be characterized, bounded, and ultimately corrected. These foundational datasets appear throughout genomic AI as training labels, evaluation benchmarks, and sometimes both simultaneously. Recognizing when the same data sources serve multiple roles is essential for interpreting model performance honestly and anticipating where generalization will fail. The evaluation and confounding issues raised by these inherited constraints are examined in depth in @sec-eval and @sec-confound.