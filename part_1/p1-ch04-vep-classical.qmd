...
4. Population databases (gnomAD) may contain carriers of pathogenic variants who haven't yet manifested disease or have incomplete penetrance
...
Ensemble principles for combining multiple predictors connect to the deep ensemble approaches for uncertainty quantification examined in @sec-ch23-deep-ensembles. Integration strategies that combine classical scores with foundation model predictions are examined in @sec-ch14-combining-evidence.
...
**Looking ahead:** The limitations of hand-crafted features motivate the shift to learned representations (@sec-ch05-embeddings), where models learn relevant patterns from data rather than expert specification. Foundation models (@sec-ch13-defining) extend this approach by pretraining on massive unlabeled datasets, enabling variant effect prediction without the label circularity that affects supervised methods (@sec-ch17-vep-fm).
