...
Protein language models (PLMs) trained on evolutionary sequence databases (see @sec-ch12-protein-lm) have emerged as effective tools for protein design, providing both generative sampling capabilities and fitness estimation for candidate sequences. The success of these approaches stems from a key insight: evolution has conducted billions of years of experiments on protein sequence space, and PLMs trained on the surviving sequences implicitly encode constraints on what works.
