...
**Prerequisites:** This chapter builds on variant effect prediction (@sec-ch17-vep-fm), protein language models (@sec-ch15-esm-family), regulatory foundation models (@sec-ch16-enformer), and network analysis (@sec-ch21-networks). Familiarity with GWAS concepts (@sec-ch03-gwas) and fine-mapping (@sec-ch03-fine-mapping) is essential. The confounding concepts from @sec-ch12-confounding are particularly relevant for understanding validation challenges.
...
![Drug target prioritization pipeline](../figs/part_6/ch29/01-fig-target-prioritization.svg)
...
Consider a typical workflow. Starting from GWAS summary statistics (see @sec-ch02-gwas-summary), statistical **fine-mapping** methods identify credible sets of potentially causal variants at each locus (see @sec-ch03-fine-mapping). Sequence-based foundation models then score each candidate variant for regulatory or coding impact. Protein-centric variant effect predictors such as *AlphaMissense*, *GPN-MSA*, and the missense components of *AlphaGenome* combine **protein language models**, structural information, and evolutionary conservation to assess coding variants (@sec-ch15-esm-family; @sec-ch14-protein-vep) [@cheng_alphamissense_2023; @benegas_gpn-msa_2024; @avsec_alphagenome_2025; @brandes_genome-wide_2023]. Regulatory foundation models including *Enformer*, *Borzoi*, and long-context DNA language models predict the consequences of noncoding variants on chromatin accessibility, transcription factor binding, and gene expression (@sec-ch16-enformer; @sec-ch14-enformer-vep).
...
From @sec-ch21-networks, recall the message passing paradigm in graph neural networks: how do nodes aggregate information from their neighbors? How might this same principle apply when propagating GWAS evidence through protein-protein interaction networks?
:::
...
![Drug-target interaction prediction with foundation models](../figs/part_6/ch29/03-fig-drug-target-prediction.svg)
...
Foundation models enhance this analysis by providing more accurate variant effect predictions (distinguishing true loss-of-function from benign variants) and by integrating across evidence types (see @sec-ch17-vep-fm). A gene might show modest individual associations with several safety-relevant traits that, when aggregated using foundation model representations, reveal a concerning pattern.
...
::: {#fig-perturb-seq}
![Perturb-seq for mechanism discovery](../figs/part_6/ch29/04-fig-perturb-seq.svg)
...
The following section covers active learning and iterative experimental design. These concepts require familiarity with uncertainty quantification (@sec-ch23-epistemic) and transfer learning (@sec-ch09-full-finetuning). Readers may wish to review these topics before proceeding.
:::
...
![Biomarker discovery pipeline with foundation models](../figs/part_6/ch29/05-fig-biomarker-pipeline.svg)
...
Beyond germline genetics, drug development increasingly leverages somatic genomics, transcriptomics, proteomics, and other molecular readouts. Tumor sequencing combined with expression profiling characterizes the molecular landscape of each patient's cancer. Single-cell multiome data (RNA plus ATAC) reveal cell-state heterogeneity that bulk assays miss (see @sec-ch19-single-cell).
...
::: {.callout-tip title="Recall and Connect"}
From @sec-ch26-regulatory, recall the challenges of model documentation for regulatory submissions and the liability chain from model developer to clinician. How do these governance considerations affect industry decisions about whether to build, buy, or fine-tune foundation models for drug discovery?
:::
...
Multi-modal foundation models jointly trained on DNA, RNA, proteins, structures, small molecules, and phenotypic readouts learn representations that span these modalities (see @sec-ch22-multi-omics). Such models can predict not just whether a molecule binds a target, but how target modulation in a particular genetic context might affect cellular phenotypes. Closed-loop optimization uses genomic foundation models to predict target relevance and liability, chemistry and protein foundation models to propose molecules, and experimental feedback to update both model types in active learning cycles.

The detailed treatment of molecular design belongs to @sec-ch30-design. From a target identification perspective, the key point is that genomic foundation models determine whether a target is worth pursuing; downstream models then optimize how to hit it. The investment in accurate target identification and validation pays dividends throughout the drug discovery pipeline by ensuring that optimization efforts focus on targets with the highest probability of clinical success.
...
**Connections:** This chapter bridges variant effect prediction (@sec-ch17-vep-fm), network analysis (@sec-ch21-networks), and confounding (@sec-ch12-confounding) to practical drug discovery applications. The concepts connect forward to molecular design (@sec-ch30-design).
:::
