...
**Your Opportunity:** Regulatory knowledge is not just compliance: it is competitive advantage. Researchers who understand what evidence FDA requires can design validation studies that satisfy regulators from the start, rather than scrambling to retrofit approval-ready documentation onto a mature model. If you are building genomic AI tools, this chapter teaches you how to get your model to patients, and how to do so responsibly.
:::
...
::: {.callout-note}
## Analytical, Clinical, and Clinical Utility: Three Levels of Validity

Medical AI evaluation distinguishes three progressively demanding levels of evidence:

- **Analytical validity:** Does the test accurately measure what it claims to measure? For a variant effect predictor, this means: given a sequence input, does the model produce consistent, reproducible scores? Can it handle different input formats, sequencing platforms, and edge cases?

- **Clinical validity:** Do the measurements correlate with clinical outcomes? A variant classifier with high analytical validity might still fail clinical validity if its "pathogenic" predictions do not correspond to actual disease risk. Clinical validity requires demonstrating that model outputs predict clinically meaningful endpoints.

- **Clinical utility:** Does using the test improve patient outcomes compared to not using it? This is the highest bar: a model might have perfect analytical and clinical validity yet offer no utility if the same clinical decisions would be made without it, or if no actionable interventions exist.

Regulatory submissions typically require strong evidence for the first two; clinical utility evidence is sometimes required for high-risk applications but is harder to generate because it demands comparative effectiveness studies.
:::
...
The FDA's action plan for AI/ML-based Software as a Medical Device (SaMD) establishes the regulatory framework for algorithms that continuously learn from real-world data [@us_food_and_drug_administration_artificial_2021]. This guidance introduces the concept of a "predetermined change control plan" (PCCP), allowing manufacturers to specify anticipated model updates without requiring new submissions for each modification.

For genomic foundation models, the PCCP framework raises important questions: How should regulators evaluate models that improve with each patient encounter? What validation is required when a model's behavior changes post-deployment? These questions remain actively debated as regulators develop genomics-specific guidance.

::: {.callout-important}
## Regulatory Uncertainty
As of 2025, no genomic foundation model has received FDA clearance for clinical diagnosis. The regulatory pathway remains uncertain, particularly for models that operate across multiple variant types or phenotypes.
:::

The FDA's evolving framework for AI/ML-based medical devices [@food_and_drug_administration_using_2023] provides additional guidance on predetermined change control plans, real-world performance monitoring, and transparency requirements. For genomic foundation models, this framework implies that developers must specify in advance what types of model updates will occur without re-submission versus which changes require new regulatory review.
...
### Laboratory-Developed Tests and the LDT Distinction {#sec-ch27-ldt}

The regulatory landscape for **laboratory-developed tests (LDTs)** further complicates matters in the United States. LDTs are diagnostic tests designed, manufactured, and used within a single clinical laboratory, as opposed to commercial test kits distributed across multiple sites. Clinical laboratories have historically been able to develop and offer these tests under their own validation without FDA premarket review, relying instead on **CLIA (Clinical Laboratory Improvement Amendments)** certification and state licensure. CLIA is the 1988 federal law establishing quality standards for laboratory testing: laboratories must meet personnel, quality control, and proficiency testing requirements to receive certification, but CLIA does not evaluate the clinical validity of individual tests the way FDA evaluates medical devices.

Many clinical genomics laboratories use in-house bioinformatics pipelines, variant callers, and annotation tools that incorporate machine learning components without seeking FDA clearance, operating under the LDT framework. Recent FDA guidance signals intent to assert greater oversight over LDTs, particularly those using complex algorithms, but the boundary between regulated devices and unregulated laboratory procedures remains contested [@food_and_drug_administration_laboratory_2024].

::: {.callout-note}
## LDT vs. SaMD: A Critical Regulatory Distinction

The distinction between **laboratory-developed tests (LDTs)** and **Software as Medical Device (SaMD)** determines which regulatory pathway applies:

| Aspect | Laboratory-Developed Test (LDT) | Software as Medical Device (SaMD) |
|--------|--------------------------------|-----------------------------------|
| **Scope** | Developed and used within a single laboratory | Commercially distributed software |
| **Primary oversight** | CLIA certification, state licensure | FDA premarket review (510(k), PMA) |
| **Validation responsibility** | Laboratory validates internally | Manufacturer demonstrates to FDA |
| **AI complexity threshold** | Historically no distinction | Risk-tiered based on decision role |
| **Current status** | FDA seeking expanded oversight | Established but evolving for AI |

**Why this matters for foundation models:** A clinical laboratory that fine-tunes a foundation model for in-house variant interpretation might argue it is an LDT requiring only CLIA compliance. The same model packaged and sold commercially would be SaMD requiring FDA clearance. This regulatory arbitrage creates incentives for laboratories to develop proprietary AI tools rather than adopt validated commercial products, potentially fragmenting quality standards across the genomic testing landscape.
:::
...
4. **Clinical Validity Evidence:**
   - *Concordance with expert panels:* Agreement with ClinVar expert-reviewed pathogenic/benign classifications (excluding VUS): auROC = 0.91 (95% CI: 0.89-0.93) on 25,847 held-out variants submitted after model training freeze
   - *Correlation with functional outcomes:* For 847 variants with functional assay data (deep mutational scanning, splicing assays), model scores correlate with measured functional impact (Spearman rho = 0.78)
   - *Prospective clinical cohort:* In a 12-month prospective study of 2,340 patients referred for hereditary cancer panel testing, model predictions for initially unclassified variants were compared against eventual expert classifications informed by segregation data, functional evidence, and clinical follow-up
   - *Subgroup analysis:* Performance stratified by gene family, variant type, and ancestry of patients (not just submitting laboratories)
   - *Comparison to predicate:* AlphaMissense vs. CADD (0.85), REVEL (0.88) on identical prospective cohort

   Note: True clinical validity would ideally demonstrate correlation with patient outcomes (did patients with variants predicted pathogenic actually develop disease?), but such longitudinal outcome data requires years of follow-up. The above evidence represents the current regulatory expectation, which emphasizes concordance with expert consensus as a surrogate for clinical outcome prediction.
...
The legal and ethical status of aggregated training data depends on how consent documents were written, how thoroughly participants understood the scope of future use, and how jurisdictions interpret secondary use provisions. When a foundation model combines sequences from UniRef (including human protein sequences contributed under varied research protocols), ClinVar (clinical variant submissions), and population biobanks (research participants who enrolled under specific institutional frameworks), no single consent framework governs the resulting model. GDPR provisions treat genetic data as a special category requiring explicit consent, but may permit research use under legitimate interest or public interest provisions with appropriate safeguards [@european_parliament_regulation_2016]. United States regulations under the Common Rule permit secondary research on properly deidentified data, but genomic data resist complete deidentification given the uniqueness of individual genomes [@health_and_human_services_federal_2018].

Even when consent technically permits model training, broader ethical questions remain. Participants who consented to genomic research in 2005 could not have anticipated that their data might train AI systems capable of generating novel sequences or predicting sensitive traits. This temporal gap between data collection and model development exposes a fundamental tension in consent frameworks: "informed" consent requires understanding what one is consenting to, yet broad consent for future research explicitly authorizes uses that cannot be specified in advance. Some bioethicists argue this makes truly informed consent for open-ended genomic research impossible in principle: participants can consent to *a process* (oversight, governance, return of results) but not to *specific applications* they cannot foresee [@manson_rethinking_2007]. **Dynamic consent** systems that allow ongoing engagement and preference updates address some concerns but are difficult to retrofit onto legacy collections and impose burdens on participants and institutions alike [@kaye_dynamic_2014].
