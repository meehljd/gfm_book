...
: Common scenarios in genomics and whether backdoor adjustment is feasible. {#tbl-backdoor-scenarios}
...
Several approaches exist at different levels of biological abstraction. At the sequence level, DNA and RNA foundation models predict effects of mutations on molecular phenotypes like chromatin accessibility, transcription factor binding, and splicing (@sec-ch18-vep-fm). These predictions are inherently counterfactual: the model compares predicted output for reference versus alternate allele. When validated against experimental perturbations, such predictions can support causal reasoning about regulatory mechanisms.
