...
- $d_k$ is the dimension of query and key vectors (typically 64 per attention head)
- $L$ is the total sequence length, with $j \in \{1, \ldots, L\}$
...
- $\alpha_{ij} \in [0, 1]$ is the attention weight from position $i$ to position $j$
- The denominator sums over all $L$ positions, ensuring $\sum_{j=1}^{L} \alpha_{ij} = 1$
- Higher scores produce higher weights; the exponential amplifies differences between scores
...
- $\text{output}_i \in \mathbb{R}^{d_v}$ is the output representation for position $i$
- $v_j \in \mathbb{R}^{d_v}$ is the value vector for position $j$, computed as $v_j = x_j W^V$
- $d_v$ is the value dimension (typically equal to $d_k$)
- $\alpha_{ij}$ are the attention weights from @eq-07-02
...
The original transformer used sinusoidal functions with different frequencies for each embedding dimension [@vaswani_attention_2023]. For position $pos$ and dimension $i$:
...
- $pos \in \{0, 1, \ldots, L-1\}$ is the position index in the sequence
- $i \in \{0, 1, \ldots, d_\text{model}/2 - 1\}$ indexes the embedding dimension pairs
- $d_\text{model}$ is the model's embedding dimension
- The factor $10000^{2i/d_\text{model}}$ creates wavelengths ranging from $2\pi$ to $10000 \cdot 2\pi$
...
*T5*-style relative position bias adds a learnable scalar to attention scores based on the distance between query and key positions [@raffel_exploring_2023]. This bias helps the model learn that nearby positions often interact more strongly than distant ones while remaining agnostic about absolute position. The learned biases can capture genomic-specific distance preferences, such as the characteristic spacing of regulatory elements or the periodicity of nucleosome positioning.
...
- **Feed-forward**: "Given what I have gathered, what should I conclude?" (per-position computation)
...
1. Multi-head attention for inter-position communication allows positions to exchange information; feed-forward network performs per-position computation to transform representations.

2. Residual connections create gradient highways that allow signals to flow directly through many layers without degradation, preventing vanishing gradients in deep networks.

3. 96 different attention patterns (12 layers x 8 heads = 96 independent attention mechanisms).

4. Pre-norm normalizes before each sublayer, providing more stable training; post-norm normalizes after, which can achieve better final performance but is harder to train.
:::
:::
...
| *DNABERT-2* [@zhou_dnabert-2_2024] | Encoder-only | 768 | 12 | 12 | 117M | Variable |
...
- You are willing to adopt newer, less mature architectures
:::
