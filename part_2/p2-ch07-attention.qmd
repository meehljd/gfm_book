...
The attention mechanism computes similarity scores between each query and all keys. For position *i*, we compute the dot product between its query *q_i* and every key *k_j* across all positions *j* = 1, ..., *L*, where *L* is sequence length. These scores are scaled by √*d_k* (the square root of the key dimension) to prevent the dot products from growing large in high dimensions, which would push softmax outputs toward extreme values and create vanishing gradients:

$$
\text{score}(q_i, k_j) = \frac{q_i \cdot k_j}{\sqrt{d_k}}
$$

A softmax function converts these scores into attention weights *α_ij* that form a probability distribution over positions:
...
These fixed patterns have useful properties. They are deterministic (the same for all sequences), allow the model to learn to attend by relative positions (since PE(*pos*+*k*) can be expressed as a linear function of PE(*pos*)), and generalize to sequence lengths not seen during training. The different frequencies across dimensions create a unique "fingerprint" for each position: lower-frequency components capture coarse position while higher-frequency components capture fine position.
...
Standard self-attention's *O*(*L*²) complexity becomes prohibitive for long genomic contexts, forcing architectural choices that trade expressiveness for tractability. The strategies employed reflect different assumptions about which interactions matter most for genomic modeling.
