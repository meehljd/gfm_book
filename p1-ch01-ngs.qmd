...
Finally, for cohorts, **joint genotyping and cohort refinement** recombines per-sample likelihoods to enforce a consistent set of variants across individuals. Variant quality score recalibration (VQSR) or hard filters distinguish true variants from artifacts based on features like depth, mapping quality, and strand bias.
...
This formulation assumes conditional independence of reads given the genotype—an assumption known to be violated in practice due to correlated errors from PCR duplicates, systematic instrument biases, and local sequence context [@depristo_gatk_2011]. DeepVariant's CNN, by contrast, sees all reads in a pileup simultaneously and can learn to model these dependencies implicitly.
...
For the purposes of this book, HLA exemplifies a broader lesson: regions of extreme diversity, structural complexity, or clinical importance may require specialized methods beyond generic variant calling pipelines. The models in later chapters that predict variant effects must be aware of—and often explicitly exclude or specially handle—these challenging regions.
...
Around each candidate site, DeepVariant constructs an image-like representation. Each row corresponds to a read overlapping the site, channels encode base identity, base quality, mapping quality, strand, and other features, and the reference sequence and candidate alleles are overlaid. This transforms the problem into a vision task: classify the central locus as homozygous reference, heterozygous, or homozygous alternate given the pileup image.

In modern versions, DeepVariant encodes each candidate variant site as a six-channel tensor. The channels capture whether each position matches or mismatches the reference, Phred-scaled base quality scores, mapping quality indicating confidence in alignment, strand orientation, which reads support the reference versus alternate allele, and additional features capturing alignment differences. This representation allows the CNN to see patterns that correlate with true variants—such as balanced allele support across strands and consistent base qualities—versus artifacts like strand-biased support or clustering of mismatches at read ends.

### Inception-Style CNN Classifier

DeepVariant uses an Inception-style CNN originally developed for image classification. Trained on high-confidence truth sets such as GIAB genomes, it learns to recognize patterns consistent with true variants (balanced alleles, consistent qualities, clean alignments) and to recognize artifacts (strand bias, mapping pileups in repeats, inconsistent quality patterns).

Once trained, the same architecture generalizes across whole-genome versus whole-exome data, PCR-free versus PCR-amplified libraries, and different instrument models and read lengths.
