...
**Prerequisites:** Understanding of transformer architectures and self-attention (@sec-ch07-attention), masked language modeling objectives (@sec-ch08-mlm), and basic protein biochemistry (amino acids, protein folding). Familiarity with DNA language models (@sec-ch14-dna-lm) provides helpful context but is not required.
...
|-----------------|-----------------|----------|
| Secondary | Attention patterns follow helix/sheet boundaries | @sec-ch15-emergent-knowledge |
...
![Attention captures residue contacts](../figs/part_3/ch15/01-A-fig-plm-emergent.svg)

![Secondary structure emerges in embedding space](../figs/part_3/ch15/01-B-fig-plm-emergent.svg)

![Attention concentrates on functional sites](../figs/part_3/ch15/01-C-fig-plm-emergent.svg)

![Evolutionary relationships encoded in embeddings](../figs/part_3/ch15/01-D-fig-plm-emergent.svg)
...
The scaling behavior mirrors observations in natural language processing, where larger models consistently capture more nuanced patterns. This predictable relationship between scale and capability provides a roadmap for model development: if more biological knowledge is needed, train a larger model on more data. The practical implications shaped how the field approached subsequent genomic foundation models, with the scaling law framework and its implications discussed in @sec-ch13-scaling.

::: {#fig-esm2-scaling layout-ncol=3}
![Loss vs. model size](../figs/part_3/ch15/02-A-fig-esm2-scaling.svg)

![Downstream performance scales consistently](../figs/part_3/ch15/02-B-fig-esm2-scaling.svg)

![ESM-2 follows compute-optimal scaling](../figs/part_3/ch15/02-C-fig-esm2-scaling.svg)
...
![ESMFold eliminates MSA requirement](../figs/part_3/ch15/03-A-fig-esmfold.svg)

![Speed vs. accuracy trade-off](../figs/part_3/ch15/03-B-fig-esmfold.svg)

![ESMFold fails for specific protein categories](../figs/part_3/ch15/03-C-fig-esmfold.svg)

![ESMFold enables proteome-scale prediction](../figs/part_3/ch15/03-D-fig-esmfold.svg)
...
*AlphaFold2's* performance at CASP14 in 2020 solved a 50-year grand challenge, predicting protein structures with accuracy competitive with experimental determination [@jumper_alphafold2_2021]. The achievement transformed structural biology and earned its creators the 2024 Nobel Prize in Chemistry. Yet *AlphaFold* is not a foundation model in the sense this book uses the term (see @sec-ch13-fm-principles). Understanding why illuminates what makes PLM-based approaches distinctive.
...
![Log-likelihood ratio scoring mechanism](../figs/part_3/ch15/04-A-fig-plm-variant-scoring.svg)

![Evolutionary intuition behind zero-shot scoring](../figs/part_3/ch15/04-B-fig-plm-variant-scoring.svg)

![Deep mutational scanning correlation](../figs/part_3/ch15/04-C-fig-plm-variant-scoring.svg)

![ClinVar pathogenicity discrimination](../figs/part_3/ch15/04-D-fig-plm-variant-scoring.svg)
...
The detailed comparison of variant effect prediction methods, including how PLM-based scores integrate with clinical classification frameworks, is covered in @sec-ch17-alphamissense. The calibration of these scores to ACMG criteria appears in @sec-ch14-acmg-mapping, and integration into rare disease diagnostic workflows in @sec-ch26-fm-scoring. Here, the key point is that protein language models provide the foundational representations that make accurate zero-shot variant prediction possible.
...
- **For integration with clinical pipelines:** PLM scores are one line of evidence among many. See @sec-ch17-vep-fm for integration with ACMG criteria and other computational evidence.
...
The limitations described in this section—orphan proteins, novel folds, epistasis—recur throughout clinical applications. In @sec-ch17-vep-fm, you'll see how these limitations affect variant pathogenicity prediction: missense variants in poorly characterized proteins receive unreliable scores. In @sec-ch28-rare-disease, rare disease diagnosis must account for these gaps when prioritizing candidate variants. The population-specific performance disparities discussed in @sec-ch12-confounding apply equally here: PLMs trained predominantly on sequences from well-studied organisms may underperform on human variants in understudied gene families.
:::

::: {#fig-plm-limitations}
![Limitations and failure modes of protein language models](../figs/part_3/ch15/05-fig-plm-limitations.svg)
...
- DNA language models (@sec-ch14-dna-lm) adapt this paradigm to genomic sequences
- Variant effect prediction methods (@sec-ch17-vep-fm) integrate PLM scores with other evidence
- Protein design (@sec-ch30-design) builds on PLM representations for generative tasks
- Interpretability methods (@sec-ch24-interpretability) probe what PLMs have learned
:::
