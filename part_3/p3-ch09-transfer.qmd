...
![Domain shift in genomic transfer learning](../figs/part_3/ch09/01-fig-domain-alignment.svg)
...
When data is severely limited (dozens of examples), practitioners face a choice between three imperfect options. Linear probing on frozen features provides the most stable approach but may miss task-specific patterns not captured in pretrained representations. Few-shot learning methods (see @sec-ch10-few-shot) attempt to adapt with minimal examples by using structured prompts or metric learning, but success varies dramatically across tasks. Collecting more data, though often expensive, may be the only path to reliable adaptation.
...
![Task relatedness determines feature relevance](../figs/part_3/ch09/02-A-fig-four-factors.svg)

![Data quantity constrains adaptation strategy](../figs/part_3/ch09/02-B-fig-four-factors.svg)

![Model expressiveness vs. overfitting risk](../figs/part_3/ch09/02-C-fig-four-factors.svg)

![Distribution overlap enables knowledge transfer](../figs/part_3/ch09/02-D-fig-four-factors.svg)
...
![Conservative escalation decision flowchart](../figs/part_3/ch09/03-fig-conservative-escalation.svg)
...
![Linear probing workflow for transfer diagnostics](../figs/part_3/ch09/04-fig-linear-probing-workflow.svg)
...
:::{.callout-note collapse="true" title="Check Your Answer"}
1. Yes, the 85% accuracy with a linear probe indicates the frozen embeddings contain substantial pathogenicity-relevant information that is linearly separable.

2. No, the minimal gains from adding complexity (only 1.5% improvement with a five-layer MLP) suggest more complex classifiers will not help much - the useful information is already captured linearly.

3. The ceiling appears to be around 86-87%, indicating that further improvements likely require updating the pretrained model itself rather than just adding more classifier capacity. This pattern suggests linear probing is sufficient and parameter-efficient fine-tuning or full fine-tuning would be needed to go beyond this ceiling.
:::
:::
