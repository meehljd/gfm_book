...
Sequence language models introduced the self-supervised pretraining paradigm (@sec-pretraining) to genomics. *DNABERT* applied **masked language modeling** to DNA sequences, demonstrating that general representations could be learned without task-specific labels. *ESM* and *ESM-2* showed that protein language models pretrained on sequence alone could transfer effectively to structure prediction, variant effect prediction, and protein design (@sec-protein-lm). The *Nucleotide Transformer* family scaled DNA language modeling to cross-species training corpora (@sec-dna-lm). *HyenaDNA* used implicit convolutions to reach million-token contexts at single-nucleotide resolution.
...
## Defining Genomic Foundation Models {#sec-ch10-defining-genomic-foundation-models}
...
### What Doesn't Count {#sec-ch10-doesn-t-count}
...
## Scaling Laws and Compute-Optimal Training {#sec-ch10-scaling-laws-compute-optimal-training}
...
### Scaling Law Framework {#sec-ch10-scaling-law-framework}
...
### Empirical Scaling in Genomic Models {#sec-ch10-empirical-scaling-genomic-models}
...
### Compute-Optimal Decisions for Genomics {#sec-ch10-compute-optimal-decisions-genomics}
...
## A Taxonomy of Genomic Foundation Models {#sec-ch10-taxonomy-genomic-foundation-models}
...
### DNA Language Models {#sec-ch10-dna-language-models}
...
### Sequence-to-Function Foundation Models {#sec-ch10-sequence-function-foundation-models}
...
*Enformer* predicts thousands of chromatin and expression tracks from 200 kb sequence windows through transformer attention (@sec-regulatory). *Borzoi* extends this with refined architectures and expanded coverage. *Sei* organizes predictions into interpretable sequence classes through unsupervised clustering. Earlier models including *DeepSEA* and *Basset* established the paradigm at smaller scales (@sec-cnn).
...
### Variant Effect Prediction Models {#sec-ch10-variant-effect-prediction-models}
...
### Multi-Omic Foundation Models {#sec-ch10-multi-omic-foundation-models}
...
Sequence diversity presents a similar tradeoff. Training on reference genomes alone provides clean sequences but limited exposure to population variation. Incorporating variant data improves robustness but requires careful design to avoid learning spurious associations. Models may also train on raw sequence alone or incorporate functional annotations, trading generality against functional grounding. The implications of training data choices for model bias are examined in @sec-confounding.


### Architecture Choices {#sec-ch10-architecture-choices}

Architectural decisions determine both computational characteristics and inductive biases. Among transformer variants, encoder-only models (*DNABERT*, *Nucleotide Transformer*) excel at classification and embedding tasks, while decoder-only models (*GROVER*) support generative applications (@sec-attention). Full and sparse attention patterns, linear approximations, and Flash attention implementations affect computational efficiency.
...
## Build Versus Use Decisions {#sec-ch10-build-versus-use-decisions}
...
### When to Use Existing Models {#sec-ch10-when-use-existing-models}
...
### When to Adapt Existing Models {#sec-ch10-when-adapt-existing-models}
...
### When to Train from Scratch {#sec-ch10-when-train-scratch}
...
### Cost-Benefit Analysis {#sec-ch10-cost-benefit-analysis}
...
## Evaluation Principles {#sec-ch10-evaluation-principles}
...
### Multi-Task Assessment {#sec-ch10-multi-task-assessment}
...
The diversity of evaluation tasks complicates comparison across models. A model excelling at promoter classification may underperform on eQTL fine-mapping. Direct comparisons require controlling for differences in training data, model scale, and evaluation protocols. Standardized benchmark suites are examined in @sec-benchmarks.


### Transfer Versus Pretraining Performance {#sec-ch10-transfer-versus-pretraining-performance}
...
Detailed discussion of benchmark suites, evaluation protocols, and methodological best practices appears in @sec-benchmarks and @sec-evaluation.


## Foundation Model Ecosystem {#sec-ch10-foundation-model-ecosystem}

Genomic foundation models exist within a broader ecosystem of infrastructure, community resources, and shared practices.


### Model Distribution {#sec-ch10-model-distribution}
...
### Documentation Requirements {#sec-ch10-documentation-requirements}
...
### Industry and Academic Contributions {#sec-ch10-industry-academic-contributions}
...
Existing models learn correlations without distinguishing causal from spurious relationships. Integrating causal structure could improve robustness and enable counterfactual reasoning, but current architectures provide no principled mechanism for causal inference (@sec-confounding).

Models trained on reference genomes and common variants may not calibrate well for ultra-rare or *de novo* variants, precisely the variants most likely to be clinically actionable (@sec-rare-disease). Improved integration of structural and evolutionary constraints could strengthen rare variant interpretation.

Translation to clinical use requires robust cross-population performance, calibrated uncertainty (@sec-uncertainty), interpretability for clinicians (@sec-interpretability), prospective validation, and regulatory approval. These requirements extend well beyond benchmark performance, and the path from research model to clinical deployment remains poorly charted.


## Convergence Without Consolidation {#sec-ch10-convergence-consolidation}

Foundation models for genomics divide into families serving different needs. DNA language models learn general sequence representations from self-supervised pretraining, capturing evolutionary constraints and regulatory patterns without explicit functional labels (@sec-dna-lm). Sequence-to-function models predict molecular phenotypes from sequence, providing quantitative outputs (expression levels, chromatin states, splice probabilities) that DNA language models alone cannot produce (@sec-regulatory). Variant effect models integrate sequence representations with evolutionary information to score the functional impact of genetic variants (@sec-vep-fm). Multi-omic models combine sequence with additional data modalities to capture regulatory relationships that sequence alone cannot resolve (@sec-multi-omics). No single family dominates; effective genomic AI requires matching model capabilities to application requirements.
...
This framework instantiates across specific domains. DNA language models (@sec-dna-lm) and protein language models (@sec-protein-lm) exemplify self-supervised pretraining on biological sequence. Regulatory models (@sec-regulatory) demonstrate sequence-to-function prediction at long-range scales. Variant effect prediction (@sec-vep-fm) integrates multiple model families for clinical interpretation. Throughout, these principles guide model selection: what does this application require, which model family provides it, and what scale is necessary to achieve it?