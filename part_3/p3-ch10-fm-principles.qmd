...
Sequence language models introduced the self-supervised pretraining paradigm (@sec-ch08-pretraining) to genomics. *DNABERT* applied **masked language modeling** to DNA sequences, demonstrating that general representations could be learned without task-specific labels [@ji_dnabert_2021]. *ESM* and *ESM-2* showed that protein language models pretrained on sequence alone could transfer effectively to structure prediction, variant effect prediction, and protein design [@rives_esm_2021; @lin_esm-2_2022] (@sec-ch12-protein-lm). The *Nucleotide Transformer* family scaled DNA language modeling to cross-species training corpora [@dalla-torre_nucleotide_2023] (@sec-ch11-dna-lm). *HyenaDNA* used implicit convolutions to reach million-token contexts at single-nucleotide resolution [@nguyen_hyenadna_2023].
