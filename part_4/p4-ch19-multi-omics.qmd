...
Foundation models address each molecular layer individually: sequence models predict regulatory effects from DNA (@sec-ch13-regulatory), expression models capture transcriptional programs (@sec-ch15-rna), and protein language models predict structure and function from amino acid sequence (@sec-ch12-protein-lm). Multi-omics integration asks how these modality-specific representations can be combined into unified patient or cell representations.
...
GLUE, introduced in @sec-ch16-single-cell for single-cell multi-omics integration, exemplifies this approach. Separate variational autoencoders encode RNA-seq and ATAC-seq data into a shared cell embedding space. A feature graph links ATAC-seq peaks to genes based on genomic proximity and **transcription factor** binding, providing biological constraints on the alignment. The result enables integration of measurements from different cells, not just different modalities in the same cell.
