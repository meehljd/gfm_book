...
> Sequence models learn what molecules can do. Network models learn what they actually do together.

:::{.callout-note}
## Chapter Overview

**Prerequisites:** This chapter builds on foundation model concepts from Part 3 (especially protein language models in @sec-ch15-protein-lm and DNA models in @sec-ch14-dna-lm) and attention mechanisms from @sec-ch07-attention. Familiarity with single-cell representations (@sec-ch19-single-cell) is helpful but not required.
...
![PPI networks: undirected physical binding](../figs/part_4/ch21/01-A-fig-biological-networks.svg)

![Gene regulatory networks: directed TF-target relationships](../figs/part_4/ch21/01-B-fig-biological-networks.svg)

![Knowledge graphs: heterogeneous multi-type nodes and edges](../figs/part_4/ch21/01-C-fig-biological-networks.svg)

![Spatial graphs: cell proximity from spatial transcriptomics](../figs/part_4/ch21/01-D-fig-biological-networks.svg)
...
Spatially resolved transcriptomics and imaging data give rise to graphs capturing tissue organization invisible to bulk or even single-cell measurements (@sec-ch19-single-cell). In these **spatial and cell-cell interaction graphs**, nodes represent cells or spatial locations, while edges encode physical proximity or inferred ligand-receptor communication. Such graphs enable questions about how spatial context influences cell behavior.
...
![Initial node features from foundation model embeddings](../figs/part_4/ch21/02-A-fig-message-passing.svg)

![Message computation along each edge](../figs/part_4/ch21/02-B-fig-message-passing.svg)

![Aggregation combines neighbor information](../figs/part_4/ch21/02-C-fig-message-passing.svg)

![After L layers, nodes capture L-hop neighborhood context](../figs/part_4/ch21/02-D-fig-message-passing.svg)
...
The central architectural insight for genomic graph learning is that foundation models and graph neural networks operate at complementary levels of abstraction. Sequence-based foundation models excel at extracting biological information from linear sequences: *ESM-2* learns **evolutionary constraints** and structural propensities from protein sequences (@sec-ch15-protein-lm); *DNABERT* and its successors capture regulatory motifs and sequence grammar (@sec-ch14-dna-lm); single-cell foundation models like *scGPT* learn cell state representations from expression profiles (@sec-ch19-single-cell). These representations encode rich biological knowledge but operate on individual entities without explicit relational information. The principles of feature extraction from pretrained models, which underpin this integration pattern, are developed in @sec-ch09-feature-extraction.

::: {#fig-gnn-integration layout-ncol=2}
![Foundation models produce rich entity representations](../figs/part_4/ch21/03-A-fig-gnn-integration.svg)

![Biological networks encode relational structure](../figs/part_4/ch21/03-B-fig-gnn-integration.svg)

![GNNs integrate embeddings with network structure](../figs/part_4/ch21/03-C-fig-gnn-integration.svg)

![Combined capabilities exceed either approach alone](../figs/part_4/ch21/03-D-fig-gnn-integration.svg)
...
The choice of integration pattern depends on data availability, computational resources, and the degree of **distribution shift** between foundation model **pretraining** and the target application (@sec-ch12-confounding). For well-characterized systems with substantial labeled data, joint fine-tuning may be warranted. For novel organisms or rare diseases with limited labels, frozen embeddings with simple GNN layers often generalize better.
...
![GWAS locus: multiple genes in LD with lead SNP](../figs/part_4/ch21/04-A-fig-disease-gene-prioritization.svg)

![Network context: one gene connects to disease genes](../figs/part_4/ch21/04-B-fig-disease-gene-prioritization.svg)

![GNN scoring prioritizes network-connected candidates](../figs/part_4/ch21/04-C-fig-disease-gene-prioritization.svg)

![Integration of sequence and network features](../figs/part_4/ch21/04-D-fig-disease-gene-prioritization.svg)
...
Clinical applications include rare disease diagnosis, where patient exome sequencing identifies hundreds of candidate variants and network-based scoring helps prioritize which genes to investigate further (@sec-ch28-rare-disease). The approach also supports drug target identification by highlighting genes whose network position and functional properties make them amenable to therapeutic modulation (@sec-ch29-drug-discovery). For rare disease diagnosis, network-based prioritization integrates with the variant filtering pipelines in @sec-ch26-prioritization-funnel, where foundation model embeddings and network context jointly inform gene ranking.
...
This application connects to broader drug discovery workflows (@sec-ch29-drug-discovery), where target identification is one component of a multi-stage pipeline. GNN-based predictions provide hypotheses for experimental validation, accelerating the search for novel therapeutic targets.
...
![Heterogeneous knowledge graph with multiple node types](../figs/part_4/ch21/05-A-fig-kg-drug-repurposing.svg)

![Multi-hop reasoning provides mechanistic hypotheses](../figs/part_4/ch21/05-B-fig-kg-drug-repurposing.svg)

![Link prediction scores potential drug-disease associations](../figs/part_4/ch21/05-C-fig-kg-drug-repurposing.svg)

![FM embeddings enhance node representations](../figs/part_4/ch21/05-D-fig-kg-drug-repurposing.svg)
...
Single-cell foundation models generate rich representations of individual cells (@sec-ch19-single-cell), but many biological questions involve relationships between cells: which cells communicate, how spatial neighborhoods influence behavior, which cell types co-occur in disease states.
...
The network modeling principles you learn here power clinical applications in later chapters. In @sec-ch28-rare-disease, network propagation prioritizes disease genes from GWAS loci—the same message passing mechanics you've learned enable identifying which genes in a locus are most likely causal. In @sec-ch29-drug-discovery, drug-target interaction prediction combines PLM embeddings (@sec-ch15-protein-lm) with network proximity to disease genes. The ascertainment bias discussed above directly affects these applications: understudied genes appear less connected than they truly are, potentially causing prioritization methods to miss important candidates.
:::
...
::: {#fig-network-bias}
![Study bias: publication count correlates with network degree](../figs/part_4/ch21/06-fig-network-bias.svg)
...
Yet network structure carries its own biases. Protein interaction databases are enriched for well-studied genes and disease-relevant pathways; less-characterized genes have fewer annotated interactions regardless of their biological importance. Correlation between genes does not imply regulatory relationship. Class imbalance between known disease genes and the genome-wide background reflects research history as much as biology. These biases propagate through GNN predictions, creating systematic patterns in what the models emphasize and what they miss. The **multi-omics** integration examined in @sec-ch22-multi-omics extends graph-based reasoning to additional modalities. Clinical applications in @sec-ch27-clinical-risk leverage network-derived features for risk stratification, while @sec-ch28-rare-disease applies network propagation to gene prioritization workflows. Both depend on understanding where network-derived predictions are trustworthy and where they inherit the limitations of their inputs, challenges that the uncertainty quantification methods in @sec-ch23-uq-methods help address.
...
- @sec-ch22-multi-omics: Extending GNNs to multi-modal data
- @sec-ch23-uncertainty: Uncertainty quantification for network predictions
- @sec-ch24-interpretability: Interpreting what GNNs learn
- @sec-ch27-clinical-risk: Clinical applications of network features
- @sec-ch28-rare-disease: Network-based gene prioritization for diagnosis
:::
