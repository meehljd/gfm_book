...
The third category comprises models optimized specifically for predicting the functional or clinical consequences of genetic variants. These models take a variant (typically represented as reference and alternative sequences or as a perturbation to a reference sequence) and predict its effect on molecular phenotypes, organismal fitness, or disease risk.

**Core characteristics.** Variant effect prediction models integrate sequence context with evolutionary information, population genetics signals, and sometimes structural or functional annotations. They output pathogenicity scores, effect size estimates, or functional consequence predictions. Training typically combines multiple data sources: clinical labels from ClinVar, population frequency data from gnomAD, functional assays such as deep mutational scanning, and evolutionary constraint metrics.

**Representative examples** span multiple eras and approaches. CADD pioneered annotation-based scoring at genome scale (@sec-cadd). AlphaMissense applies protein language models to predict pathogenicity of missense variants across the human proteome [@cheng_alphamissense_2023]. ESM-1v uses evolutionary context for protein variant effect prediction. EVE combines evolutionary and structural information for variant interpretation. Genomic foundation models like DNABERT and Enformer derivatives provide variant effect predictions through in silico mutagenesis, comparing predictions between reference and alternative alleles.

The architecture, training strategies, evaluation, and clinical deployment of variant effect predictors are covered comprehensively in @sec-vep. That chapter examines the progression from feature-based methods through CNN-based approaches to modern foundation models, discusses integration of multiple evidence sources, and addresses the persistent challenges of calibration, generalization, and clinical translation. For clinical application workflows, see @sec-variants.
...
Evaluating genomic foundation models requires standardized benchmarks that enable fair comparison across architectures, training regimes, and modeling approaches. Rather than catalog specific benchmarks here, we direct readers to @sec-benchmarks, which provides comprehensive coverage of benchmark suites across all genomic modeling domains: molecular and regulatory prediction, variant effect prediction, protein structure and function, and trait-level prediction.

Key benchmark categories include DNA-level tasks (regulatory element classification, enhancer-promoter linking, transcript abundance prediction), protein-level tasks (variant effect prediction via deep mutational scanning, structure prediction), and trait-level tasks (polygenic score evaluation, complex trait prediction). @sec-benchmarks discusses the design principles, coverage, and limitations of each major benchmark suite, including ProteinGym, TraitGym, Nucleotide Transformer benchmarks, and comparative evaluations across foundation models.

The methodological principles for how to use these benchmarks appropriately—including data splitting strategies, metric selection, and pitfalls to avoid—are covered in @sec-eval. Together, these chapters provide the evaluation infrastructure and best practices needed to assess genomic foundation models rigorously.
