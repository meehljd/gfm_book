...
::: {#fig-scaling-laws}
![Scaling laws for genomic foundation models](../figs/part_7/ch32/04-fig-scaling-laws.svg)
...
*Hint: Review the model taxonomy from @sec-ch14-taxonomy and the specific model chapters if needed.*

:::{.callout-note collapse="true" title="Check Your Answer"}
1. **ESM-2**: Protein sequence/structure scale (amino acid level)
2. **Enformer**: Regulatory element scale (kilobase DNA sequences predicting gene expression)
3. **scGPT**: Single-cell scale (cellular gene expression states)
4. **Akita**: Chromosome-scale (3D genome organization and chromatin contacts)
5. **AlphaMissense**: Protein variant scale (missense mutation effects on protein function)

Each excels at its primary scale but does not explicitly integrate across scales.
:::
:::

Architectures that explicitly integrate across scales remain a frontier. Hierarchical models that compose representations at different resolutions, graph neural networks that encode biological relationships across scales (@sec-ch22-canonical-architectures), and hybrid systems that combine modality-specific encoders with cross-modal attention layers all represent active research directions.
...
![Multiscale integration for future foundation models](../figs/part_7/ch32/01-fig-multiscale-integration.svg)
...
::: {.callout-tip title="Recall and Connect"}
Chapter 25 covered causal inference in depth. Before examining the table below, retrieve from memory: What are the three main approaches to causal inference in genomics discussed in @sec-ch26-causal, and what is the fundamental limitation of each? How do these limitations affect foundation model training?
...
|----------|-----------|-------------|-------------------|
| **Mendelian randomization** | Uses genetic variants as instruments for causal inference | Requires valid instruments; pleiotropy confounds | @sec-ch26-mendelian-randomization |
| **Perturbation screens** | Direct experimental intervention (CRISPR, drugs) | Expensive; context-specific; off-target effects | @sec-ch26-crispr-screens |
| **Structural causal models** | Explicit DAG representation of causal relationships | Requires prior knowledge; difficult to scale | @sec-ch26-causal-discovery |
| **Counterfactual prediction** | Model what would happen under intervention | Training data observational; extrapolation risk | @sec-ch26-counterfactual-limits |
...
![Correlation versus causation in genomic AI](../figs/part_7/ch32/05-fig-correlation-causation.svg)
...
::: {#fig-agentic-systems}
![Agentic scientific systems for autonomous discovery](../figs/part_7/ch32/02-fig-agentic-systems.svg)
...
Current genomic foundation models largely operate on single modalities: DNA sequence, protein sequence, gene expression counts, chromatin accessibility signals. Biological reality is irreducibly multimodal, with information flowing across modalities through transcription, translation, signaling, and metabolism. The next generation of genomic foundation models will need to integrate across modalities more deeply, building on the multi-omic approaches discussed in @sec-ch23-multi-omics.
...
See @sec-ch23-strategies for detailed integration strategies.
:::
...
These questions connect to the biosecurity considerations in @sec-ch27-biosecurity and require governance frameworks that evolve with technical capabilities.
:::
...
The foundation model paradigm offers particular advantages for learning health systems. Pretrained models can be adapted to local populations and practices through fine-tuning on institutional data (@sec-ch10-full-finetuning; @sec-ch13-domain-adaptation). Improvements demonstrated at one institution can potentially transfer to others through shared model updates. Common architectures enable comparison across sites and accumulation of evidence across diverse populations.
...
| VUS reclassified through outcomes | — | Hundreds of variants | — |
...
![Learning health system integrating genomic foundation models](../figs/part_7/ch32/03-fig-learning-health-system.svg)
...
- It performs well on average but poorly for underrepresented populations (@sec-ch13-fairness)
- Its predictions are uncalibrated and clinicians cannot interpret confidence levels (@sec-ch24-calibration)
...
::: {#fig-translation-gap}
![Translation gap between benchmarks and clinical impact](../figs/part_7/ch32/06-fig-translation-gap.svg)
...
This illustrates **domain shift** within the scaling/efficiency challenge (training data bias) and the **translation gap** (equity failures). Mitigation: @sec-ch10-domain-shift for domain-adaptive fine-tuning; @sec-ch13-confounding for detecting ancestry confounding; @sec-ch13-ancestry-confounding for multi-ancestry approaches.
:::
...
- Tiered consent for learning loop (@sec-ch27-consent)
- Equity monitoring across hospital populations (@sec-ch27-fairness)
- FDA breakthrough device pathway for evolving system (@sec-ch27-validation)
...
- Multi-scale integration builds on single-cell (@sec-ch20-single-cell), 3D genome (@sec-ch21-3d-genome), and network approaches (@sec-ch22-networks)
- Causality challenges extend the discussion from @sec-ch26-causal
- Governance requirements connect to @sec-ch27-regulatory and responsible development practices
